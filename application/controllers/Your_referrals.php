        ...
        foreach ($level_2_list as $level_2) {
          ...
          if ($this->ReferralModel->count_referrals($level_2->referee_id) > 0) {
            ...
            foreach ($level_3_list as $level_3) {
              ...
              if ($this->ReferralModel->count_referrals($level_3->referee_id) > 0) {
                $level_4_list = $this->ReferralModel->get_referees($level_3->referee_id);
                // print_r($level_3_list);

                foreach ($level_4_list as $level_4) {
                  $total_downline++;

                  if ($this->Members->get_member_by_id($level_4->referee_id) != null) {

                    $referral4 = array();

                    $level_4_data = $this->Members->get_member_by_id($level_4->referee_id);



                    if ($this->DepositModel->has_active_deposit($level_4->referee_id)) {
                      $total_active++;

                      $total_deposit_4 = $this->DepositModel->get_total_deposit($level_4->referee_id);
                  ...
                  if ($this->ReferralModel->count_referrals($level_4->referee_id) > 0) {
                    $level_5_list = $this->ReferralModel->get_referees($level_4->referee_id);
                    // print_r($level_3_list);

                    foreach ($level_5_list as $level_5) {
                      $total_downline++;

                      if ($this->Members->get_member_by_id($level_5->referee_id) != null) {

                        $referral5 = array();

                        $level_5_data = $this->Members->get_member_by_id($level_5->referee_id);



                        if ($this->DepositModel->has_active_deposit($level_5->referee_id)) {
                          $total_active++;

                          $total_deposit_5 = $this->DepositModel->get_total_deposit($level_5->referee_id);
                      ...
                      if ($this->ReferralModel->count_referrals($level_5->referee_id) > 0) {
                        $level_6_list = $this->ReferralModel->get_referees($level_5->referee_id);
                        // print_r($level_3_list);

                        foreach ($level_6_list as $level_6) {
                          $total_downline++;

                          if ($this->Members->get_member_by_id($level_6->referee_id) != null) {

                            $referral6 = array();

                            $level_6_data = $this->Members->get_member_by_id($level_6->referee_id);



                            if ($this->DepositModel->has_active_deposit($level_6->referee_id)) {
                              $total_active++;

                              $total_deposit_6 = $this->DepositModel->get_total_deposit($level_6->referee_id);
                          ...
                          // if ($this->ReferralModel->count_referrals($level_6->referee_id) > 0) {
                          //   $level_7_list = $this->ReferralModel->get_referees($level_6->referee_id);
                          //   // print_r($level_3_list);

                          //   foreach ($level_7_list as $level_7) {
                          //     $total_downline++;

                          //     if ($this->Members->get_member_by_id($level_7->referee_id) != null) {

                          //       $referral7 = array();

                          //       $level_7_data = $this->Members->get_member_by_id($level_7->referee_id);



                          //       if ($this->DepositModel->has_active_deposit($level_7->referee_id)) {
                          //         $total_active++;

                          //         $total_deposit_7 = $this->DepositModel->get_total_deposit($level_7->referee_id);
                          ...
                          //     if ($this->ReferralModel->count_referrals($level_7->referee_id) > 0) {
                          //       $level_8_list = $this->ReferralModel->get_referees($level_7->referee_id);
                          //       // print_r($level_3_list);

                          //       foreach ($level_8_list as $level_8) {
                          //         $total_downline++;

                          //         if ($this->Members->get_member_by_id($level_8->referee_id) != null) {

                          //           $referral8 = array();

                          //           $level_8_data = $this->Members->get_member_by_id($level_8->referee_id);



                          //           if ($this->DepositModel->has_active_deposit($level_8->referee_id)) {
                          //             $total_active++;

                          //             $total_deposit_8 = $this->DepositModel->get_total_deposit($level_8->referee_id);
                          ...
                          //         if ($this->ReferralModel->count_referrals($level_8->referee_id) > 0) {
                          //           $level_9_list = $this->ReferralModel->get_referees($level_8->referee_id);
                          //           // print_r($level_3_list);

                          //           foreach ($level_9_list as $level_9) {
                          //             $total_downline++;

                          //             if ($this->Members->get_member_by_id($level_9->referee_id) != null) {

                          //               $referral9 = array();

                          //               $level_9_data = $this->Members->get_member_by_id($level_9->referee_id);



                          //               if ($this->DepositModel->has_active_deposit($level_9->referee_id)) {
                          //                 $total_active++;

                          //                 $total_deposit_9 = $this->DepositModel->get_total_deposit($level_9->referee_id);
                          ...
                          //             if ($this->ReferralModel->count_referrals($level_9->referee_id) > 0) {
                          //               $level_10_list = $this->ReferralModel->get_referees($level_9->referee_id);
                          //               // print_r($level_3_list);

                          //               foreach ($level_10_list as $level_10) {
                          //                 $total_downline++;

                          //                 if ($this->Members->get_member_by_id($level_10->referee_id) != null) {

                          //                   $referral10 = array();

                          //                   $level_10_data = $this->Members->get_member_by_id($level_10->referee_id);



                          //                   if ($this->DepositModel->has_active_deposit($level_10->referee_id)) {
                          //                     $total_active++;

                          //                     $total_deposit_10 = $this->DepositModel->get_total_deposit($level_10->referee_id);
