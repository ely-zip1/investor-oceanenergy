...
class Withdraw extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $account_balance = ($total_growth + $total_bonus + $total_received) - $total_withdrawal->amount - $total_reinvestment->amount - $total_sent - $pending_withdrawal->total;
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->load->view('templates/footer');
        ...
        if($_POST['plan_payment_mode'] == 'mode1'){
          ...
          $withdrawal_data['payment_method'] = 'Stellar';
          if(strlen($withdrawal_modes->stellar) <= 0){
            redirect('account_settings', 'refresh');
          }
  			}
  ...
  public function valid_amount()
  {
    ...
    if($_POST['withdraw_amount'] < 20){
      $this->form_validation->set_message('valid_amount', 'Invalid amount.');
      return false;
    }else if($_POST['withdraw_amount'] > $account_balance){
      $this->form_validation->set_message('valid_amount', 'Invalid amount.');
			return false;
    }else {
      return true;
