...
class Plans extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['mode7'] = ucfirst($modes_of_payment[4]->name);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if($_POST['plan_payment_mode'] == 'mode1'){
				...
				$deposit_data['deposit_options_id'] = $modes_of_payment[4]->id;
	...
	function valid_deposit(){
		...
		if($_POST['chosen_plan'] == 'plan1'){
			...
			if($_POST['deposit_amount'] >= 800 && $_POST['deposit_amount'] <= 7999){
				return true;
			}else {
    		$this->form_validation->set_message('valid_deposit', 'Deposit Amount does not match your selected plan.');
				return false;
			...
			if($_POST['deposit_amount'] >= 8000 && $_POST['deposit_amount'] <= 80000){
				return true;
			}else {
    		$this->form_validation->set_message('valid_deposit', 'Deposit Amount does not match your selected plan.');
				return false;
