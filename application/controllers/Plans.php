...
class Plans extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['mode7'] = ucfirst($modes_of_payment[5]->name);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($_POST['plan_payment_mode'] == 'mode1') {
				...
				$deposit_data['deposit_options_id'] = $modes_of_payment[5]->id;
	...
	function valid_deposit()
	{
		...
		if ($_POST['chosen_plan'] == 'plan1') {
			...
			if ($_POST['deposit_amount'] >= 1500 && $_POST['deposit_amount'] <= 6499) {
				return true;
			} else {
				$this->form_validation->set_message('valid_deposit', 'Deposit Amount does not match your selected plan.');
				return false;
			...
			if ($_POST['deposit_amount'] >= 7500 && $_POST['deposit_amount'] <= 20000) {
				return true;
			} else {
				$this->form_validation->set_message('valid_deposit', 'Deposit Amount does not match your selected plan.');
				return false;
				...
				if ($this->ReferralModel->get_referrer($level_2->referrer_id)->referrer_id != 1) {
					...
					if ($this->ReferralModel->get_referrer($level_3->referrer_id)->referrer_id != 1) {
						...
						if ($this->ReferralModel->get_referrer($level_4->referrer_id)->referrer_id != 1) {
							...
							if ($this->ReferralModel->get_referrer($level_5->referrer_id)->referrer_id != 1) {
