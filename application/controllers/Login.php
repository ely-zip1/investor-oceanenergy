...
class Login  extends CI_Controller
{
	...
	{
		// $ref = $this->Referral_codes->get_all();

		// print_r($ref);


		// foreach ($ref as $codes) {
		// 	$this->Referral_codes->set_referral_link('https://dashboard-lombardmedical.com/my/ref/'.$codes->code, $codes->id);
		// }
		...
		//
		// 			$countr_name = str_replace(' ','', $this->session->country);
		//
		// 			print_r($filename);
		// 	}
		// 	closedir($handle);
				...
				if($this->is_login_valid()){
					...
					if ($handle = opendir($dir)) {
						while (false !== ($file = readdir($handle))) {
								...
								$country_name = str_replace(' ','', $userdata['country']);

								if(strtolower(substr($filename, 0, 4)) == strtolower(substr($country_name, 0, 4))){
									// print_r($filename);
									// print_r($countr_name);
									$userdata['flag'] = $file;
								}
